...
public class MyReveal extends Activity {
    ...
    public boolean isRevealOpen = false;
    /* 动画 */
    int DURATION = 500;//动画持续时间
    int RADIUS = 10;

    public MyReveal(Context context) {
        mContext = context;
    }

    public void setRevealViewLayout(RevealView revealViewLayout) {
        this.mLayoutRevealView = revealViewLayout;
    }
     ...
     * @param p 水波中心坐标
     */
    public void showLayoutRevealColorView(Point p) {
     ...
     * @param p
     */
    public void closeLayoutRevealColorView(Point p) {
//        Point p = getLocationInView(mLayoutRevealView, mFloatingView);
    ...
    public void hideReveal() {
        mLayoutRevealView.setVisibility(View.GONE);
        isRevealOpen = false;
    }
    ...
    public int getThemeColor() {
        ...
        TypedArray a = mContext.obtainStyledAttributes(typedValue.data, colorAttr);
