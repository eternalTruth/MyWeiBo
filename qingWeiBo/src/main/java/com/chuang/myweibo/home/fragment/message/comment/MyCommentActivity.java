...
public class MyCommentActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_message_commentlist_layout);
    ...
    public void initRecyclerView() {
        ...
        mAdapter = new MyCommentAdapter(mContext, mDatas);
...
//                    onLoadNextPage(recyclerView);
    ...
    public void pullToRefreshData() {
        mSwipeRefreshLayout.setRefreshing(true);
//        mNext_cursor = 0;
        MainFragment.mCommentsAPI.toME(
                0,//since_id
                0,//max_id
                NewFeature.GET_COMMENT_NUMS,//count
                1,//page
                0,//authorType
                0,//sourceType
                new RequestListener() {
                    ...
                    public void onComplete(String response) {
                        ...
                        if (!TextUtils.isEmpty(response)) {
                            if (NewFeature.CACHE_MESSAGE_COMMENT) {
                                SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + "/qingwb/", "message_comment.txt", response);
                            }
//                            mNext_cursor = Integer.valueOf(UserList.parse(response).next_cursor);
                    ...
                    public void onWeiboException(WeiboException e) {
                        if (NewFeature.CACHE_MESSAGE_COMMENT) {
                            String response = SDCardUtil.get(mContext, SDCardUtil.getSDCardPath() + "/qingwb/", "message_comment.txt");
                            mDatas = CommentList.parse(response).commentList;
                            updateList();
                        }
                        mSwipeRefreshLayout.setRefreshing(false);
