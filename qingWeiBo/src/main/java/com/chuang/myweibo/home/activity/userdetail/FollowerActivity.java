...
public class FollowerActivity extends Activity implements View.OnClickListener {
    ...
    public void initRecyclerView() {
        mRecyclerView = (RecyclerView) findViewById(R.id.recyclerview_follower);
...
//        mHeaderAndFooterRecyclerViewAdapter = new HeaderAndFooterRecyclerViewAdapter(mAdapter);
    ...
    public void pullToRefreshData() {
        mSwipeRefreshLayout.setRefreshing(true);
        mNext_cursor = 0;
        MainFragment.mFriendshipsAPI.followers(
                ...
                new RequestListener() {
                    ...
                    public void onComplete(String response) {
                        Log.d("113", "onComplete: "+response);
                        //短时间内疯狂请求数据，服务器会返回数据，但是是空数据。为了防止这种情况出现，要在这里要判空
                        if (!TextUtils.isEmpty(response)) {
                            if (NewFeature.CACHE_WEIBOLIST) {
                                SDCardUtil.put(mContext, SDCardUtil.getSDCardPath() + "/qingwb/", "我的粉丝列表缓存.txt", response);
                            }
