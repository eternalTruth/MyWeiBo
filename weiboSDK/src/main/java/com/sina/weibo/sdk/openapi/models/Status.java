...
public class Status {
    ...
    public ArrayList<String> pic_urls_bmiddle;//自己添加
    ...
    public static Status parse(JSONObject jsonObject) {
        ...
        if (picUrlsArray != null && picUrlsArray.length() > 0) {
            ...
            status.pic_urls_bmiddle = new ArrayList<String>(length);///
            String tempUrl = null;//
            JSONObject tmpObject = null;
            for (int ix = 0; ix < length; ix++) {
                tmpObject = picUrlsArray.optJSONObject(ix);
                if (tmpObject != null) {
                    tempUrl = tmpObject.optString("thumbnail_pic");

                    status.pic_urls.add(tempUrl);
                    status.pic_urls_bmiddle.add(getOriginUrl(tempUrl));///
                }
            }
        }
    ...
    /**
     * 自定义获取中等清晰图片
     * 清晰度从第到高依次为：thumbnail(90)，bmiddle(440),mw690(690),mw1024(1024),large(原图)
     */
    private static String getOriginUrl(String url) {
        StringBuffer buffer = new StringBuffer(url);
        //http://ww4.sinaimg.cn/thumbnail/75b1a75fjw1f2zi1dhyluj20ck0ckdhf.jpg
        //t的下标22，l下标30
